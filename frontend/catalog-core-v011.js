(() => {
  ...
  const prettyId=value=>String(value||'').replace(/^(NGC|IC)(\d+)/i,'$1 $2');
  ...
  const ngcDsos=D.dsos.map(r=>({key:`dso:${String(r[0]).replace(/\s+/g,'')}`,category:'dso',kind:TYPES[r[5]]||'Deep-sky object',id:r[0],name:r[1]||prettyId(r[0]),commonName:r[1]||'',raHours:r[2],decDeg:r[3],mag:r[4],type:r[5],constellation:r[6]||'—',majorArcmin:r[7],minorArcmin:r[8],positionAngleDeg:r[9],aliases:Array.isArray(r[10])?r[10]:[r[0]],source:'OpenNGC'}));
  ...
  const rows=ext.map(o=>{const values=[o.name,o.id,prettyId(o.id),o.designation,o.hip?`hip ${o.hip}`:'',o.constellation,o.kind,...(o.aliases||[])].filter(Boolean);return{o,text:values.join(' ').toLowerCase(),compactText:values.map(compact).join(' ')};});
  ...
  const oldDisplay=objectDisplayName;objectDisplayName=o=>o?.category==='dso'?(o.commonName?`${prettyId(o.id)} · ${o.commonName}`:prettyId(o.id)):oldDisplay(o);
  ...
  const oldSearch=searchObjects;searchObjects=(query,category)=>{
    const q=String(query||'').trim().toLowerCase(),qc=compact(q),base=oldSearch(query,category);if(category==='planet'||category==='messier')return base;
    const found=[];for(const r of rows){if(category!=='all'&&r.o.category!==category)continue;if(q&&!r.text.includes(q)&&!r.compactText.includes(qc))continue;let rank=3;const a=[r.o.id,prettyId(r.o.id),r.o.name,r.o.designation,...(r.o.aliases||[])].filter(Boolean).map(x=>String(x).toLowerCase());if(q&&a.some(x=>x===q||compact(x)===qc))rank=0;else if(q&&a.some(x=>x.startsWith(q)||compact(x).startsWith(qc)))rank=1;found.push({o:r.o,rank});if(found.length>300)break;}
  ...
  window.noctemCatalogV011={version:D.version,meta:{...D.meta,caldwellSupplementCount:caldwellSupplement.length},stars,dsos,ngcDsos,caldwellSupplement,byKey,constellations:D.constellations,sizeText,prettyId,typeName:t=>TYPES[t]||'Deep-sky object'};
})();
