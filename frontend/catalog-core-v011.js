(() => {
  ...
  const dsos=D.dsos.map(r=>({key:`dso:${String(r[0]).replace(/\s+/g,'')}`,category:'dso',kind:TYPES[r[5]]||'Deep-sky object',id:r[0],name:r[1]||r[0],commonName:r[1]||'',raHours:r[2],decDeg:r[3],mag:r[4],type:r[5],constellation:r[6]||'—',majorArcmin:r[7],minorArcmin:r[8],positionAngleDeg:r[9],aliases:Array.isArray(r[10])?r[10]:[r[0]],source:'OpenNGC'}));
  ...
  window.noctemCatalogV011={version:D.version,meta:D.meta,stars,dsos,byKey,constellations:D.constellations,sizeText,typeName:t=>TYPES[t]||'Deep-sky object'};
})();
